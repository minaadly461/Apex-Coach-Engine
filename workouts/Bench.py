...
def process_bench_press(input_video_path, output_video_path, chart_output_path):
    print(f"[ENGINE] Analyzing Bench Press with Stabilizer: {input_video_path}")
    ...
    CONF_THRESHOLD = 0.5 # تجاهل القفزات الناتجة عن ضعف ثقة الموديل
    ...
    # الزوايا المعدلة بناءً على تجاربك
    ...
    while cap.isOpened():
        ...
        if results[0].keypoints is not None and len(results[0].keypoints.xy[0]) > 10:
            ...
            # تثبيت النقط: لو الثقة ضعيفة، استخدم آخر متوسط معروف
            ...
            # رسم الهيكل العظمي (أحمر باهت + نقط بيضاء)
            ...
            if target[0][0] != 0:
                ...
                # --- منطق العد الذكي (Smart State Machine) ---
                if angle > EXTENSION_THRESHOLD:
                    if stage == "down": # تم إكمال العدة بالصعود
                        reps += 1
                ...
                # عرض البيانات على الفريم
                draw_text(frame, f"Angle: {int(angle)} deg", (30, 60), color=(0, 255, 255))
                draw_text(frame, f"Bench Reps: {reps}", (30, 110), color=(0, 255, 0))
    ...
    if rep_durations:
        ...
        ax1.set_title('TIME PER REP (BENCH)', fontweight='bold', pad=15)
        for bar in bars:
            yval = bar.get_height()
    ...
    if angles_history:
        ...
        ax2.set_title('BENCH PRESS ANGLE TRACKER', fontweight='bold', pad=15)
