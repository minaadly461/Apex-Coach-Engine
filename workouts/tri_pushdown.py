...
def start_workout(target_reps=10):
    ...
    UPPER_THRESH, LOWER_THRESH = 150, 65
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if avg_sway > 15 or avg_drift > 15:
                    if current_rep_form == "Good":
                        current_rep_form = "Bad"
                        bad_reps_list.append(f"Rep #{good_reps+bad_reps+1}: Elbows flared or swinging detected.")
                ...
                if avg_angle > UPPER_THRESH:
                    if stage == "up":
                        ...
                        stage, current_rep_form, rep_start_time = "down", "Good", time.time()
                    elif stage == "Waiting...":
                        stage, current_rep_form = "down", "Good"
                        
                if avg_angle < LOWER_THRESH and stage == "down":
                    stage = "up"
            ...
            cv2.imshow('Apex Coach - Triceps Pushdown', image)
            if good_reps >= target_reps or cv2.waitKey(10) & 0xFF == ord('q'): break
                
    cap.release()
    cv2.destroyAllWindows()
    plot_analytics(angles_history, rep_durations, 'TRICEPS: TIME PER REP (SEC)', 'TRICEPS: EXTENSION TRACKER', bad_reps_list)